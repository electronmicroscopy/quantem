...
class TomographyNerf:
    ...
    def train(
        ...
        if self.global_rank == 0:
            print(
                f"Using projection {zero_tilt_idx} (angle={self.tomo_dataset.tilt_angles[zero_tilt_idx]:.2f}°) as reference"
            )
            ...
            for batch_idx, batch in enumerate(self.dataloader):
                ...
                with torch.autocast(
                    ...
                    if tv_weight > 0:
                        ...
                        tv_indices = torch.randperm(all_coords.shape[0], device=all_coords.device)[
                            :num_tv_samples
                        ]
            ...
            if epoch % viz_freq == 0 or epoch == epochs - 1 or epoch % checkpoint_freq == 0:
                with torch.no_grad():
                    ...
                    metrics = torch.tensor(
                        [avg_loss, avg_mse_loss, avg_tv_loss, avg_z1_loss], device=self.device
                    )
                    if self.world_size > 1:
