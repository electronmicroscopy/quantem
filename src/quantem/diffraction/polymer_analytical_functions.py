...
# Generalized helper functions, to be moved somewhere else for common use
def add_to_histogram_bilinear(
    histogram: np.ndarray,
    x: Union[float, np.ndarray],
    y: Union[float, np.ndarray],
    bins: Union[float, np.ndarray],
    ...
    """
    Add weighted values to a histogram with bilinear spatial interpolation and 
    linear bin interpolation.
    
    This function performs trilinear interpolation to distribute values across 
    histogram bins, accounting for fractional positions in both spatial dimensions 
    and the binning dimension. Operates in-place on the histogram array.
    
    Parameters
    ----------
    histogram : np.ndarray
        The histogram array to update. Must have at least 3 dimensions.
        Shape: (..., height, width, num_bins) or permuted based on bin_axis.
    x : float or np.ndarray
        X-coordinate(s) in the spatial grid. Can be fractional.
        If array, must be broadcastable with y, bins, and weights.
    y : float or np.ndarray
        Y-coordinate(s) in the spatial grid. Can be fractional.
        If array, must be broadcastable with x, bins, and weights.
    bins : float or np.ndarray
        Bin index/indices (can be fractional) for the third dimension.
        If array, must be broadcastable with x, y, and weights.
    weights : float or np.ndarray, optional
        Weight values for each entry. If None, uses weight of 1.0.
        If array, must be broadcastable with x, y, and bins.
    bin_axis : int, optional
        Axis of the histogram corresponding to the bin dimension (default: -1).
    periodic : bool, optional
        Whether the bin dimension is periodic (e.g., angles). If True, bins
        wrap around (default: True).
    bounds_mode : str, optional
        How to handle out-of-bounds spatial coordinates:
        - 'clip': Clip to valid range (default)
        - 'ignore': Skip out-of-bounds entries
    ...
    -------
    None
        Operates in-place on the histogram array.
    
    Raises
    ------
    ValueError
        If histogram doesn't have enough dimensions, if inputs are incompatible,
        or if bounds_mode='error' and coordinates are out of bounds.
    
    Notes
    -----
    The interpolation works as follows:
    
    1. **Spatial interpolation (bilinear)**: Each value is distributed among 
       the 4 nearest spatial cells weighted by distance.
    
    2. **Bin interpolation (linear)**: Within each spatial cell, the value is 
       distributed between adjacent bins weighted by the fractional bin position.
    
    3. **Total distribution**: Each input value affects up to 8 histogram cells
       (4 spatial × 2 bin positions).
    
    For periodic bins (e.g., angles), the bin index automatically wraps using 
    modulo arithmetic. For non-periodic bins, out-of-range values are clipped.
    
    Examples
    --------
    >>> # Create a 3D histogram: 10x10 spatial grid, 36 angular bins
    >>> hist = np.zeros((10, 10, 36))
    >>> 
    >>> # Add a single point at fractional position
    >>> add_to_histogram_bilinear(
    ...     hist,
    ...     x=5.3,
    ...     y=7.8,
    ...     bins=12.6,  # Between bins 12 and 13
    ...     weights=1.5,
    ...     periodic=True
    ... )
    >>> 
    >>> # Add multiple points at once (vectorized)
    >>> x_coords = np.array([2.5, 3.7, 8.1])
    >>> y_coords = np.array([4.2, 5.9, 1.3])
    >>> bin_indices = np.array([10.0, 25.5, 32.8])
    >>> intensities = np.array([1.0, 2.0, 0.5])
    >>> 
    >>> add_to_histogram_bilinear(
    ...     hist,
    ...     x=x_coords,
    ...
    ... )
    >>> 
    >>> # For orientation histograms (angles in radians)
    >>> orient_hist = np.zeros((100, 100, 180))  # 180 degree bins
    >>> theta_deg = 45.7
    >>> add_to_histogram_bilinear(
    ...     orient_hist,
    ...     x=50.0,
    ...     y=50.0,
    ...     bins=theta_deg,  # Fractional degree
    ...     weights=1.0,
    ...     periodic=True
    ... )
    ...
    See Also
    --------
    numpy.histogram2d : 2D histogram without interpolation
    scipy.ndimage.map_coordinates : General N-D interpolation
    """
    from typing import Union, Optional
    
    # Validate histogram dimensions
    if histogram.ndim < 3:
        raise ValueError(
            f"Histogram must have at least 3 dimensions, got {histogram.ndim}"
        )
    
    # Move bin axis to the end for consistent indexing
    if bin_axis != -1 and bin_axis != histogram.ndim - 1:
        histogram = np.moveaxis(histogram, bin_axis, -1)
    
    # Get histogram shape
    if histogram.ndim == 3:
        height, width, num_bins = histogram.shape
        spatial_shape = (height, width)
    else:
        # For higher dimensional histograms, assume last 3 dims are [height, width, bins]
        spatial_shape = histogram.shape[-3:-1]
        height, width = spatial_shape
        num_bins = histogram.shape[-1]
    
    # Convert inputs to arrays for uniform handling
    x = np.atleast_1d(x)
    y = np.atleast_1d(y)
    ...
    try:
        x, y, bins, weights = np.broadcast_arrays(x, y, bins, weights)
    except ValueError as e:
        raise ValueError(
            f"Could not broadcast inputs: x{x.shape}, y{y.shape}, "
            f"bins{bins.shape}, weights{weights.shape}"
        ) from e
    ...
    for i in range(len(x_flat)):
        ...
        y_val = y_flat[i]
        bin_val = bins_flat[i]
        weight_val = weights_flat[i]
        
        # Handle out-of-bounds spatial coordinates
        if bounds_mode == "clip":
            x_val = np.clip(x_val, 0, height - 1.001)
            y_val = np.clip(y_val, 0, width - 1.001)
        elif bounds_mode == "ignore":
            if x_val < 0 or x_val >= height or y_val < 0 or y_val >= width:
                continue
        elif bounds_mode == "wrap":
            x_val = x_val % height
            y_val = y_val % width
        elif bounds_mode == "error":
            if x_val < 0 or x_val >= height or y_val < 0 or y_val >= width:
                raise ValueError(
                    f"Coordinates ({x_val}, {y_val}) out of bounds "
                    f"for histogram shape {spatial_shape}"
                )
        else:
            raise ValueError(
                f"Invalid bounds_mode: {bounds_mode}. "
                f"Choose from 'clip', 'ignore', 'wrap', 'error'"
            )
        
        # Calculate spatial interpolation
        x_floor = int(np.floor(x_val))
        y_floor = int(np.floor(y_val))
        dx = x_val - x_floor
        dy = y_val - y_floor
        
        # Ensure we don't go out of bounds after floor
        x_floor = min(x_floor, height - 2)
        y_floor = min(y_floor, width - 2)
        
        # Calculate bin interpolation
        bin_floor = np.floor(bin_val).astype(int)
        dbin = bin_val - bin_floor
        
        # Bilinear spatial weights
        spatial_weights = [
            (1 - dx) * (1 - dy),  # Bottom-left
            (dx) * (1 - dy),       # Bottom-right
            (1 - dx) * (dy),       # Top-left
            (dx) * (dy),           # Top-right
        ]
        ...
        # Distribute across spatial and bin dimensions
        for spatial_weight, (xi, yi) in zip(spatial_weights, positions):
            # Lower bin
            if periodic:
                bin_lower = bin_floor % num_bins
            else:
                bin_lower = np.clip(bin_floor, 0, num_bins - 1)
            
            if histogram.ndim == 3:
                histogram[xi, yi, bin_lower] += (
                    spatial_weight * (1 - dbin) * weight_val
                )
            else:
            ...
            # Upper bin
            if periodic:
                bin_upper = (bin_floor + 1) % num_bins
            else:
                bin_upper = np.clip(bin_floor + 1, 0, num_bins - 1)
            
            if histogram.ndim == 3:
                histogram[xi, yi, bin_upper] += (
                    spatial_weight * dbin * weight_val
                )
            else:
                histogram[..., xi, yi, bin_upper] += (
                    spatial_weight * dbin * weight_val
...
def make_orientation_histogram(
    ...
    """
    # unwrap Vector if needed
    if isinstance(bragg_peaks, np.ndarray) and bragg_peaks.dtype == object and len(bragg_peaks) == 1:
        bragg_peaks = bragg_peaks.item()
