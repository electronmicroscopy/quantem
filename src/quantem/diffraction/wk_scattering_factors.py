...
def compute_WK_factor(
    ...
    # for the middle branch, silently dropping every term with argu in
    # [0.1, 1) and producing non-monotonic form factors at small g. The
    # original EMsoftLib/others.f90 WEKO uses ARGU >= 0.1, restored here.
    WK = np.zeros_like(S)
    for i in range(4):
        argu = B[i] * S**2
        sub = argu < 0.1
        WK[sub] += A[i] * B[i] * (1.0 - 0.5 * argu[sub])
        sub = np.logical_and(argu >= 0.1, argu <= 20.0)
