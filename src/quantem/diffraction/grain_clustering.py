...
def extract_signals(
    ...
    **stored polar** ``theta`` (radians) is used directly -- optionally sign-flipped
    ...
    ----------
    bragg_peaks : object
        Provides ``.polar_peaks`` (field-indexable, ``.shape == (Rx, Ry)``, each cell a
        1-D array) and ``.peak_intensities``; or is itself such a field-indexable
    ...
    Rx, Ry = polar.shape
    R = polar[r_field]
    TH = polar[theta_field]
    II = inten_src[intensity_field]
    ...
    for rx in range(Rx):
        for ry in range(Ry):
            p_r = R[rx, ry]
            if p_r is None or len(p_r) == 0:
                continue
            p_r = np.asarray(p_r, dtype=float)
            p_th = np.asarray(TH[rx, ry], dtype=float)
            inten = np.asarray(II[rx, ry], dtype=float)
            r2 = p_r ** 2
