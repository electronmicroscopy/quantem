...
from quantem.diffraction.scan_mask_editor import ScanMaskEditor
from quantem.diffraction.peak_detection import (
    detect_blobs,
    find_central_beam_from_peaks,
    estimate_peak_windows as _estimate_peak_windows,
...
from quantem.diffraction.peak_visualization import (
    ...
    visualize_peak_detection as _visualize_peak_detection,
...
class BraggPeaksPolymer(AutoSerialize):
    ...
    def visualize_peak_detection(self, *args, **kwargs):
        """Contact sheet of detection results across several patterns.

        Five columns per pattern: polar, polar with peaks, cartesian with
        peaks, cartesian original, cartesian normalized. Forwards to
        :func:`quantem.diffraction.peak_visualization.visualize_peak_detection`.
        """

        return _visualize_peak_detection(
            self.dataset_cartesian,
            self.peak_coordinates_cartesian,
            self.polar_peaks,
            self.polar_data,
            self.image_centers,
            self.num_annular_bins,
            getattr(self, "two_fold_symmetry", False),
            *args,
            **kwargs,
        )

    def estimate_peak_windows(self, *args, **kwargs):
        """Detect the radial peak families and estimate their q-windows.

        Forwards to
        :func:`quantem.diffraction.peak_detection.estimate_peak_windows`,
        supplying this analysis's polar peaks and intensities.
        """

        return _estimate_peak_windows(
            self.polar_peaks, self.peak_intensities, *args, **kwargs
        )
