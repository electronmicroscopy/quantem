...
class FactorizedDiffractionTomography:
    # ------------------------------------------------------------------ init
    def __init__(
        self,
        real_shape: tuple[int, int, int],          # (N_z, N_y, N_x)
        k_shape: tuple[int, int, int],              # (N_kz, N_ky, N_kx)
        real_sampling: tuple[float, float, float],  # (dz, dy, dx)  [A/px]
        k_sampling: tuple[float, float, float],      # (dkz, dky, dkx) [A^-1/px]
        n_weights: int = 1,
        energy: float = 3.0e5,
        device: str = "cpu",
        seed: int = 0,
    ...
    def make_propagator(self, antialias_fraction: float = 0.9, antialias_softness: float = 0.05):
        ...
        self.prop = (bare * mask).to(self.device)
    ...
    def reconstruct(
        ...
        Full-batch gradient (accumulated over DP chunks for bounded memory) with
        an amplitude (sqrt-intensity) loss. Every ``reset_every`` iters the
        worst ``reset_fraction`` of voxels (by angle-gradient magnitude) get a
        fresh random orientation and equalized weights, to escape stuck angles.
        """
        pos = self.scan_positions(scan_shape, scan_step, scan_origin)
        n_row, n_col = pos.shape[:2]
        meas = measurements.to(self.device)
        meas_amp = meas.clamp_min(0).sqrt()
        # flat list of (tilt, j, i) jobs
        ...
        opt = torch.optim.Adam(self.parameters(), lr=lr, eps=1e-30)
        ...
        for it in range(num_iters):
            ...
            for c0 in range(0, n_dp, chunk):
                ...
                # recompute basis/rotations per chunk so each chunk's graph is
                # independent -> backward frees it -> memory stays bounded, while
                # grads still accumulate into the leaf params' .grad (full-batch).
                basis = self.masked_basis()
                R_all = self.rotation_matrices()
                cl = 0.0
                for local, (ti, j, i) in enumerate(chunk_jobs):
                    Psi = self.forward_ray(pos[j, i], float(tilts_deg[ti]), basis, R_all,
                                           self.weights, phase_only=phase_only)
                    amp = Psi.abs()
                    r = ((amp - meas_amp[ti, j, i]) ** 2).mean()
                    cl = cl + r
            ...
            # bad-voxel reset: voxels whose rays carry the most residual error get
            # a fresh random orientation + equalized weights (escape stuck angles).
            did_reset = 0
            if reset_every and (it + 1) % reset_every == 0 and it < num_iters - 1:
                err_vox = Wmat.t() @ res_per_dp                       # (n_voxels,)
                k = max(1, int(self.reset_count(reset_fraction)))
                bad = torch.topk(err_vox, k).indices
                with torch.no_grad():
                    newM = torch.eye(3).reshape(1, 3, 3).repeat(k, 1, 1) \
                        + 0.1 * torch.randn(k, 3, 3, generator=gen)
                    self.angles.M[bad] = newM.to(self.device)
                    Wflat = self.weights.reshape(self.n_voxels, self.n_weights)
                    Wflat[bad] = Wflat.mean()
                    self._reset_optimizer_state(opt, self.angles.M, bad)
                    self._reset_optimizer_state(opt, self.weights, bad)
                did_reset = k

            opt.step()
            if verbose and (it % max(1, num_iters // 20) == 0 or it == num_iters - 1):
                msg = f"  it {it:4d}  loss {mean_loss:.4e}"
                if did_reset:
                    msg += f"  (reset {did_reset} voxels)"
    ...
    def reset_count(self, fraction: float) -> int:
        return int(round(fraction * self.n_voxels))
