...
class StrainMapAutocorrelation(AutoSerialize):
    ...
    # def fit_lattice_vectors_torch(
    #     self,
    #     refine_gaussian: bool = False,
    #     refine_all_peaks: bool = False,
    #     refine_radius_px: float = 2.0,
    #     upsample: int = 16,
    #     gaussian_maxfev: int = 100,
    ...
    #     data_all = torch.from_numpy(self.dataset.array).to(device, dtype=torch.float32)
    #     mask_diffraction_torch = torch.from_numpy(self.mask_diffraction).to(device, dtype=torch.float32)
    #     mask_diffraction_inv_torch = torch.from_numpy(self.mask_diffraction_inv).to(device, dtype=torch.float32)

    #     data_all = data_all * mask_diffraction_torch + mask_diffraction_inv_torch

    #     if mode == "linear":
    #         im_all_torch = torch.fft.fftshift(torch.abs(torch.fft.fft2(data_all)), dim=(-2, -1))
    #     elif mode == "log":
    #         im_all_torch = torch.fft.fftshift(torch.abs(torch.fft.fft2(torch.log1p(data_all))), dim=(-2, -1))
    #     elif mode == "gamma":
    #         im_all_torch = torch.fft.fftshift(torch.abs(torch.fft.fft2(torch.pow(torch.clamp(data_all, min=0.0), g))), dim=(-2, -1))
    #     else:
    #         raise ValueError("metadata['mode'] must be 'linear', 'log', or 'gamma'")
        
    #     u_fit_all_torch = torch.zeros((scan_r, scan_c, 5), dtype=torch.float32, device=device)
    #     v_fit_all_torch = torch.zeros((scan_r, scan_c, 5), dtype=torch.float32, device=device)
    #     for r, c in it:
    #         u_fit_abs, v_fit_abs,_ ,_ = _refine_lattice_vectors(
    #             im_all_torch[r,c],
    #             u_rc=u0,
    ...
    #             maxfev=gaussian_maxfev,
    #         )
    #         u_fit_all_torch = u_fit_abs
    #         v_fit_all_torch = v_fit_abs

    #     # u_fit_all = u_fit_all.cpu().numpy().reshape(scan_r, scan_c, 5)
    #     # v_fit_all = v_fit_all.cpu().numpy().reshape(scan_r, scan_c, 5)
    #     u_fit_all_torch = u_fit_all_torch.cpu().numpy()
    #     v_fit_all_torch = v_fit_all_torch.cpu().numpy()

    #     self.u_peak_fit.array[:, :, :] = u_fit_all_torch
    #     self.v_peak_fit.array[:, :, :] = v_fit_all_torch

    #     self.u_fit.array[:, :, :2] = u_fit_all_torch[:2]
    #     self.v_fit.array[:, :, :2] = v_fit_all_torch[:2]
...
# def _refine_lattice_vectors_torch(
#     im: torch.Tensor,
#     *,
#     u_rc: NDArray,
#     v_rc: NDArray,
...
#         peaks = torch.as_tensor(peaks, dtype=torch.float32, device=device)
#     if weights is not None:
#         weights = torch.as_tensor(weights, dtype=torch.float32, device=device)
#     u_rc = torch.as_tensor(u_rc, dtype=torch.float32, device=device)
#     v_rc = torch.as_tensor(v_rc, dtype=torch.float32, device=device)
...
#             return 0.0
#         return np.clamp(delta, -1.0, 1.0)

#     def _parabolic_peak_rc_amp(*, r_guess: float, c_guess: float) -> tuple[float, float, float]:
#         r0 = int(torch.clip(int(torch.round(r_guess)), 0, H - 1))
#         c0 = int(torch.clip(int(torch.round(c_guess)), 0, W - 1))
#         win = im[
#             max(0, r0 - 1) : min(H, r0 + 2),
#             max(0, c0 - 1) : min(W, c0 + 2),
#         ]
...
#         ir, ic = torch.unravel_index(torch.argmax(win), win.shape)
#         r_peak = max(0, r0 - 1) + ir
#         c_peak = max(0, c0 - 1) + ic
...
#         r_sub = r_ref + dr
#         c_sub = c_ref + dc
#         r_int = int(torch.clip((torch.round(r_sub)), 0, H - 1))
#         c_int = int(torch.clip((torch.round(c_sub)), 0, W - 1))
#         amp = im[r_int, c_int]
...
#         bg0 = torch.median(win)
#         amp0 = win[ir, ic] - bg0
#         sig0 = max(0.75, radius_px / 2.0)

#         rr = torch.arange(r1, r2, dtype=float)[:, None]
#         cc = torch.arange(c1, c2, dtype=float)[None, :]
#         RR = torch.broadcast_to(rr, win.shape)
#         CC = torch.broadcast_to(cc, win.shape)
...
#         ) -> Tensor:
#             r, c = coords
#             sig = torch.maximum(sigma, 1e-12)
#             return background + amp * torch.exp(-((r - row) ** 2 + (c - col) ** 2) / (2.0 * sig * sig))
...
#         bounds_lo = (rlo, clo, 0.0, 0.25, -np.inf)
#         bounds_hi = (rhi, chi, torch.inf, radius_px * 4.0, torch.inf)

#         try:
#             popt, _ = curve_fit(
#                 _g2,
#                 (torch.ravel(RR), torch.ravel(CC)),
#                 torch.ravel(win),
#                 p0=p0,
#                 bounds=(bounds_lo, bounds_hi),
#                 maxfev=maxfev,
#             )
#             row, col, amp, sig, bg = popt
#             if not (torch.isinf(row) and torch.isinf(col) and torch.isinf(amp) and torch.isinf(sig) and torch.isinf(bg)):
#                 return r0, c0, p0[2], 0.0, 0.0
#             return row, col, amp, sig, bg
#         except Exception:
#             return r0, c0, p0[2], 0.0, 0.0

#     def _refine_one(vec: Tensor) -> Tensor:
#         vec = torch.tensor(vec, dtype=float).reshape(2)
#         r_guess = r_center + vec[0]
#         c_guess = c_center + vec[1]
...
#     def _find_initial_peaks_weights(initial_peaks: Tensor) -> tuple[Tensor, Tensor, Tensor]:
#         if initial_peaks is None:
#             threshold = torch.quantile(im, threshold_percentile)
#             conditions = (
#                 im > torch.roll(im, shifts=(-1, -1), dims=(0, 1)),
#                 im > torch.roll(im, shifts=(-1,  0), dims=(0, 1)),
#                 im > torch.roll(im, shifts=(-1,  1), dims=(0, 1)),
#                 im > torch.roll(im, shifts=( 0, -1), dims=(0, 1)),
#                 im > torch.roll(im, shifts=( 0,  1), dims=(0, 1)),
#                 im > torch.roll(im, shifts=( 1, -1), dims=(0, 1)),
#                 im > torch.roll(im, shifts=( 1,  0), dims=(0, 1)),
#                 im > torch.roll(im, shifts=( 1,  1), dims=(0, 1)),
#                 im > threshold,
#             )
#             p = torch.stack(conditions, dim=0).all(dim=0)
#             initial_peaks = torch.argwhere(p)
#             r0 = torch.tensor([r_center, c_center])
#             initial_peaks = initial_peaks - r0
        
#         peak_sp = torch.zeros(initial_peaks.shape)
#         weights = torch.zeros((initial_peaks.shape[0], 1))
#         it = 0
#         for peak in initial_peaks:
#             refined_peak = torch.as_tensor(_refine_one(peak))
#             peak_sp[it, :] = refined_peak[:2]
#             weights[it] = refined_peak[2]
#             it += 1
#         return peak_sp, weights
...
#         A = torch.column_stack((u_rc, v_rc))
#         ab0_float = torch.linalg.lstsq(A, pts.T, rcond=None)[0]
#         ab0 = (torch.round(ab0_float)).T

#         weights /= weights.sum()
#         A = torch.ones((pts.shape[0], 3))
#         A[:,:2] = ab0
#         pts_weighted = pts * torch.sqrt(weights)
#         A_weighted = A * torch.sqrt(weights)
#         uvr0 = torch.linalg.lstsq(A_weighted, pts_weighted, rcond=None)[0]
#         u_refined = uvr0[0,:]
#         v_refined = uvr0[1,:]
#         _,_, amp_par_u = _parabolic_peak_rc_amp(r_guess=u_refined[0], c_guess=u_refined[1])
#         _,_, amp_par_v = _parabolic_peak_rc_amp(r_guess=v_refined[0], c_guess=v_refined[1])
