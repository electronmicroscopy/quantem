...
class Dataset(AutoSerialize):
    ...
    def __getitem__(self, index) -> "Dataset":
        ...
        new_origin = np.asarray(self.origin)[kept_axes] if self.origin.ndim > 0 else self.origin
        new_sampling = (
            np.asarray(self.sampling)[kept_axes] if self.sampling.ndim > 0 else self.sampling
        )
        new_units = [self.units[i] for i in kept_axes] if len(self.units) > 0 else self.units

        # Pick return class: if subclass defines per-dim behavior, use that
        cls = type(self)
