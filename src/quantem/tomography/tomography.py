...
from quantem.core.ml.loss_functions import (
    CharbonnierLoss,
    L1Loss,
    LLMSELoss,
    MSELogMSELoss,
...
from quantem.tomography.object_models import ObjectINN, ObjectVoxelwise
from quantem.tomography.tomography_base import TomographyBase
from quantem.tomography.tomography_conv import TomographyConv
from quantem.tomography.tomography_ddp import (
    AdaptiveSmoothL1LossDDP,
...
class Tomography(TomographyConv, TomographyML, TomographyBase, TomographyDDP):
    ...
    def sirt_recon(
        ...
        print(
            "Devices",
            sirt_tilt_series.device,
            proj_forward.device,
            self.dataset.tilt_angles.device,
        )
    ...
    def pretrain(
        ...
        if consistency_criterion[0].lower() == "mse":
            ...
            consistency_loss_fn = AdaptiveSmoothL1LossDDP(
                beta_init=consistency_criterion[1], ema_factor=0.99, eps=1e-8
            )
        ...
        for epoch in range(epochs):
            ...
            if epoch % viz_freq == 0 or epoch == epochs - 1:
                ...
                with torch.no_grad():
                    self.obj.create_volume(
                        world_size=self.world_size,
                        global_rank=self.global_rank,
                        ray_size=volume_dataset.N,
                    )
                    ...
                    metrics = torch.tensor(
                        [avg_loss, avg_consistency_loss, avg_tv_loss], device=self.device
                    )
                ...
                if self.global_rank == 0:
                    ...
                    self.temp_logger.add_scalar(
                        "Pretrain/consistency_loss", avg_consistency_loss, epoch
                    )
                    ...
                    ax[1].matshow(
                        pred_full[volume_dataset.N // 2].cpu().numpy(), cmap="turbo", vmin=0
                    )
                    ...
                    plt.close(fig)
                    print(
                        f"Epoch [{epoch}/{epochs}] Pretrain Total Loss: {avg_loss:.6f}, Consistency Loss: {avg_consistency_loss:.6f}, TV Loss: {avg_tv_loss:.6f}"
                    )
    ...
    def recon(
        ...
        if self.global_rank == 0:
            print(
                f"Using projection {zero_tilt_idx} (angle={self.dataset.tilt_angles[zero_tilt_idx]:.2f}°) as reference"
            )
        ...
        for epoch in range(epochs):
            ...
            if self.global_rank == 0 and self.global_epochs > 0:
                ...
                if num_samples_per_ray != prev_samples:
                    print(
                        f"Epoch {epoch}: Changing num_samples_per_ray from {prev_samples} to {num_samples_per_ray}"
                    )
            ...
            for batch_idx, batch in enumerate(self.dataloader):
                ...
                with torch.autocast(
                    ...
                    ray_densities = all_densities.view(
                        len(target_values), num_samples_per_ray
                    )  # Reshape rays and integarte
                ...
                for key, opt in self.optimizers.items():
                    if key == "model":
                        model_norm = torch.nn.utils.clip_grad_norm_(
                            self.obj.model.parameters(), max_norm=1
                        )
                    elif key == "aux_params":
                        aux_norm = torch.nn.utils.clip_grad_norm_(
                            self.dataset.auxiliary_params.parameters(), max_norm=1
                        )
                    opt.step()
                    opt.zero_grad()
            ...
            if epoch % viz_freq == 0 or epoch == epochs - 1 or epoch % checkpoint_freq == 0:
                with torch.no_grad():
                    ...
                    z3 = z3.detach().cpu()
                    metrics = torch.tensor(
                        [avg_loss, avg_consistency_loss, avg_tv_loss, avg_z1_loss],
                        device=self.device,
                    )
                    if self.world_size > 1:
                        dist.all_reduce(metrics, op=dist.ReduceOp.AVG)
            ...
            if hasattr(self, "val_dataloader") and self.val_dataloader is not None:
                val_loss = self.validate(
                    aux_params, num_samples_per_ray, device_type, autocast_dtype, use_amp, N
                )
            ...
            if self.global_rank == 0 and (epoch % viz_freq == 0 or epoch == epochs - 1):
                with torch.no_grad():
                    ...
                    # Log metrics
                    self.temp_logger.add_scalar(
                        f"train/consistency_loss_{consistency_criterion[0] if isinstance(consistency_criterion, tuple) else consistency_criterion}",
                        avg_consistency_loss,
                        self.global_epochs,
                    )
                    if hasattr(self, "val_dataloader") and self.val_dataloader is not None:
                        self.temp_logger.add_scalar(
                            f"val/consistency_loss_{consistency_criterion[0] if isinstance(consistency_criterion, tuple) else consistency_criterion}",
                            val_loss,
                            self.global_epochs,
                        )
                    ...
                    self.temp_logger.add_scalar(
                        "train/model_grad_norm", model_norm.item(), self.global_epochs
                    )
                    self.temp_logger.add_scalar(
                        "train/aux_grad_norm", aux_norm.item(), self.global_epochs
                    )
                    self.temp_logger.add_scalar("train/lr", current_lr, self.global_epochs)
                    self.temp_logger.add_scalar(
                        "train/num_samples_per_ray", num_samples_per_ray, self.global_epochs
                    )
                    if consistency_criterion[0].lower() == "adaptive_smooth_l1":
                        self.temp_logger.add_scalar(
                            "train/beta_2", consistency_loss_fn.beta2.item(), self.global_epochs
                        )
                    ...
                    self.temp_logger.add_figure(
                        "train/auxiliary_params", fig, self.global_epochs, close=True
                    )
                    plt.close(fig)
