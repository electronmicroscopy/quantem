import numpy as np
from scipy.spatial import ConvexHull, Delaunay, cKDTree  # type: ignore
...
def beamlet_weights(parent_wave_vectors, wave_vectors, gpts, sampling):
    ...
    kx_flat, ky_flat = kx.numpy().ravel(), ky.numpy().ravel()
    k = np.asarray((kx_flat, ky_flat)).T

    indices = np.argmax(np.all(np.isclose(wv_np[:, None, :], k), axis=2), axis=1)
    weights = np.zeros((len(pwv_np),) + kx.shape)
