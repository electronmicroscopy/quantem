...
def _bragg_for_plotting():
    ...
    for i in range(2):
        for j in range(2):
            peaks[i, j] = np.array(
                [[4.0, 4.0], [2.0, 6.0], [6.0, 2.0]], dtype=float
            )
            polar[i, j] = np.array(
                [[0.1, 0.0], [0.4, np.pi / 2], [0.7, np.pi]], dtype=float
            )
            intensities[i, j] = np.array([[10.0], [20.0], [30.0]], dtype=float)
...
def test_bragg_peak_polar_transform_inverts_ellipse_mapping():
    ...
    peaks[0, 0] = np.array([[8.0, 14.0]], dtype=float)
    bp = BraggPeaksPolymer.from_data(
        ds,
        device="cpu",
        compute_parameters=lambda x, **kwargs: (0.0, 1.0),
...
def _bragg_orientation_histogram(theta_values, theta_step_deg=90, normalize_stack=False):
    ...
    polar[0, 0] = np.column_stack([np.ones_like(theta_values), theta_values])
    intensities = Vector.from_shape(
        shape=(1, 1),
        fields=["intensities"],
        units=["counts"],
    ...
    intensities[0, 0] = np.ones((theta_values.size, 1), dtype=float)
...
def test_bragg_private_helpers_characterize_shared_plotting_behavior():
    ...
    assert _central_peak_index(peaks_x, peaks_y, peaks_r, (3.0, 3.0)) == 0
